...
async def get_athlete_profile(
    ...
    try:
        async with ICUClient(config) as client:
            ...
            # Fetch today's wellness for CTL/ATL/TSB (not available on athlete endpoint)
            ...
            if atl is not None:
                fitness["atl"] = round(atl, 1)
            if tsb is not None:
                fitness["tsb"] = round(tsb, 1)
            if ramp_rate is not None:
                fitness["ramp_rate"] = round(ramp_rate, 1)
            ...
            if athlete.sport_settings:
                for sport in athlete.sport_settings:
                    ...
                    if sport.types:
                        sport_data["type"] = sport.types[0] if len(sport.types) == 1 else sport.types
                    if sport.ftp:
...
async def get_fitness_summary(
    ...
    try:
        async with ICUClient(config) as client:
            ...
            # Fetch today's wellness for CTL/ATL/TSB
            ctl = atl = tsb = ramp_rate = None
            try:
                today = date.today().isoformat()
                wellness = await client.get_wellness_for_date(today)
